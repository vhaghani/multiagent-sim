...
@dataclass(frozen=True)
class ExtrapParams:
    weights: tuple = (0.33, 0.25, 0.18, 0.14, 0.10)
    center: float = 0.04
    max_dev: float = 0.03
    squeeze: float = 0.1
...
@dataclass(frozen=True)
class GridParams:
    max_price_move: float = 0.2  # computed if None
    max_grid_move: float | None = None  # computed if None
    dense_limit: float = 0.05
    anticip_step_size: float = 0.01
    ...
    def resolve(self, periods_per_year: int) -> GridParams:
        ...
        if self.max_price_move is not None and self.max_grid_move is not None:
            return self
        targets = {1: 0.8, 4: 0.6, 12: 0.4, 48: 0.3, 240: 0.2}
    ...
    def build_price_grid(self) -> np.ndarray:
        ...
        pct = -self.dense_limit
        while pct <= self.dense_limit + 1e-10:
            grid.append(pct)
            pct += self.dense_step

        # Sparse region: positive side (doubling step sizes)
        step = self.dense_step * 2
        pct = self.dense_limit + step
        while pct <= self.max_grid_move:
            grid.append(pct)
            step *= 2
            pct += step
        grid.append(self.max_grid_move)  # always include the boundary
...
def print_all_parameters(params: SimulationParams):
    """Render all input parameters to a PNG file."""
    lines = []
    lines.append("SIMULATION PARAMETERS")
    lines.append("=" * 50)

    lines.append("\n── SimulationParams (top-level) ──")
    lines.append(f"  periods_per_year         = {params.periods_per_year}")
    lines.append(f"  years                    = {params.years}")
    lines.append(f"  init_exp_ret             = {params.init_exp_ret}")
    lines.append(f"  init_earnings_yield      = {params.init_earnings_yield}")
    lines.append(f"  earnings_vol             = {params.earnings_vol}")
    lines.append(f"  payout_ratio             = {params.payout_ratio}")
    lines.append(f"  max_excess_earnings_growth = {params.max_excess_earnings_growth}")
    lines.append(f"  crra                     = {params.crra}")
    lines.append(f"  stock_vol_assumed        = {params.stock_vol_assumed}")
    lines.append(f"  inout_vol                = {params.inout_vol}")
    lines.append(f"  short_max                = {params.short_max}")
    lines.append(f"  lev_max                  = {params.lev_max}")
    lines.append(f"  min_wealth               = {params.min_wealth}")
    lines.append(f"  include_anticip          = {params.include_anticip}")

    f = params.fractions
    lines.append("\n── InvestorFractions ──")
    lines.append(f"  static_frac              = {f.static_frac}")
    lines.append(f"  cashflow_frac            = {f.cashflow_frac}")
    lines.append(f"  extrap_frac              = {f.extrap_frac}")
    lines.append(f"  stmr_frac                = {f.stmr_frac}")
    lines.append(f"  mmntm_frac               = {f.mmntm_frac}")
    lines.append(f"  valmo_frac               = {f.valmo_frac}")

    r = params.real_rate
    lines.append("\n── RealRateParams ──")
    lines.append(f"  target                   = {r.target}")
    lines.append(f"  mr_speed                 = {r.mr_speed}")
    lines.append(f"  vol                      = {r.vol}")
    lines.append(f"  min_rate                 = {r.min_rate}")
    lines.append(f"  earnings_corr            = {r.earnings_corr}")
    lines.append(f"  init                     = {r.init}")
    lines.append(f"  bond_maturity            = {r.bond_maturity}")

    e = params.extrap
    lines.append("\n── ExtrapParams ──")
    lines.append(f"  weights                  = {e.weights}")
    lines.append(f"  center                   = {e.center}")
    lines.append(f"  max_dev                  = {e.max_dev}")
    lines.append(f"  squeeze                  = {e.squeeze}")
    lines.append(f"  pct_monthly              = {e.pct_monthly}")

    s = params.stmr
    lines.append("\n── StmrParams ──")
    lines.append(f"  lookback_time            = {s.lookback_time}")
    lines.append(f"  baseline                 = {s.baseline}")
    lines.append(f"  deviation                = {s.deviation}")
    lines.append(f"  numb_sigmas_max_dev      = {s.numb_sigmas_max_dev}")

    m = params.momentum
    lines.append("\n── MomentumParams ──")
    lines.append(f"  lookback_time            = {m.lookback_time}")
    lines.append(f"  baseline                 = {m.baseline}")
    lines.append(f"  deviation                = {m.deviation}")

    st = params.static
    lines.append("\n── StaticParams ──")
    lines.append(f"  baseline                 = {st.baseline}")

    v = params.valmo
    lines.append("\n── ValmoParams ──")
    lines.append(f"  lookback_time            = {v.lookback_time}")
    lines.append(f"  baseline                 = {v.baseline}")
    lines.append(f"  momentum_weight          = {v.momentum_weight}")
    lines.append(f"  rp_max                   = {v.rp_max}")

    iss = params.issuance
    lines.append("\n── IssuanceParams ──")
    lines.append(f"  issuance_threshold       = {iss.issuance_threshold}")
    lines.append(f"  buyback_threshold        = {iss.buyback_threshold}")
    lines.append(f"  issuance_rate            = {iss.issuance_rate}")
    lines.append(f"  buyback_rate             = {iss.buyback_rate}")

    g = params.grid
    lines.append("\n── GridParams ──")
    lines.append(f"  max_price_move           = {g.max_price_move}")
    lines.append(f"  max_grid_move            = {g.max_grid_move}")
    lines.append(f"  dense_limit              = {g.dense_limit}")
    lines.append(f"  anticip_step_size        = {g.anticip_step_size}")
    lines.append(f"  dense_step               = {g.dense_step}")
    lines.append(f"  step_size                = {g.step_size}")

    a = params.anticip
    lines.append("\n── AnticipParams ──")
    lines.append(f"  baseline                 = {a.baseline}")
    lines.append(f"  risk_aversion            = {a.risk_aversion}")
    lines.append(f"  initial_wealth           = {a.initial_wealth}")
    ...
    plt.savefig("parameters.png", dpi=150, bbox_inches="tight",
                facecolor="white")
    plt.close()
    print("Parameters saved to parameters.png")
...
def investor_demands(
    ...
    if t >= valmo_lb:
        ...
        momentum_frac = np.where(
            return_pa > params.init_earnings_yield,          # <-- CHANGED
...
def anticip_forecast_clearing_price(
    ...
    if t + 1 >= valmo_lb:
        ...
        momentum_frac = np.where(
            ret_pa_valmo > params.init_earnings_yield,       # <-- CHANGED
            params.valmo.momentum_weight * params.valmo.baseline,
            -params.valmo.momentum_weight * params.valmo.baseline,
        )
        rp_val = ey - nr
...
def calculate_statistics(
    ...
    # --- Trading volume ---
    # counting just one side of each trade
    total_volume = 0.5 * sum(
...
def plot_earnings_yields(
    ...
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig("earnings_yields.png", dpi=150, bbox_inches="tight")
    plt.show()
    plt.close()
...
def plot_first_simulation(
    ...
    fig.tight_layout()
    plt.savefig("equity_allocations.png", dpi=150, bbox_inches="tight")
    plt.show()
    plt.close()
    ...
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig("ey_rp_rate.png", dpi=150, bbox_inches="tight")
    plt.show()
    plt.close()
    ...
    fig.tight_layout()
    plt.savefig("price_and_earnings.png", dpi=150, bbox_inches="tight")
    plt.show()
    plt.close()
...
print_all_parameters(mc_params)
...
plot_price_paths(all_results, mc_params, max_paths=10)
plot_earnings_yields(all_results, mc_params, max_paths=10)
