...
class RealRateParams:
    ...
    init: float = 0.02
    bond_maturity: float = 10.0  # in years, 0 means no duration
...
@dataclass(frozen=True)
class StmrParams:
    lookback_time: float = 1 / 12  # in years
    baseline: float = 0.5
    deviation: float = 1 / 6
...
def investor_demands(
    ...
    # STMR: short-term mean reversion
    stmr_lb = params.stmr.lookback_periods(ppy)
    if t >= stmr_lb:
        lookback_tr = market.tr_price[t - stmr_lb]
        ret_pa = (trial_tr_prices / lookback_tr) ** (ppy / stmr_lb) - 1
        stmr_alloc = np.where(
            ret_pa < params.init_earnings_yield,
            params.stmr.baseline + params.stmr.deviation,
            params.stmr.baseline - params.stmr.deviation,
        )
...
def anticip_forecast_clearing_price(
    ...
    # STMR: lookback uses t+1 as the forecast index
    stmr_lb = params.stmr.lookback_periods(ppy)
    if t + 1 >= stmr_lb:
